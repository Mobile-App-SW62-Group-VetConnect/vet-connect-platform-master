...
public class UpdateVetServiceCommandFromResourceAssembler {
    public static UpdateVetServiceCommand toCommandFromResource(UpdateVetServiceResource resource, Long id) {
        return new UpdateVetServiceCommand(id, resource.name(), resource.description(), resource.price(), resource.duration(), resource.category(), resource.features(), resource.isActive());
    }
}
